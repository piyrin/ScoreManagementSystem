...
enum class AdminOpResult
{
    ...
    RELATED_DATA_EXIST, // 存在关联数据（无法删除，如教师已授课/学生有成绩）
    SYSTEM_ERROR        // 系统错误
};
...
public:
    ...
    // 按角色查询所有用户
    std::vector<UserModel> queryAllUsers(const UserModel &login_admin, int roleFilter = 0);
    ...
    // 统计数据
    //全局数据统计（用户数+课程数）
    std::pair<std::tuple<int, int, int>, int> statGlobalData(const UserModel &login_admin);

    //整体成绩统计
    std::pair<double, double> statAllScores(const UserModel &login_admin);
